...
%display(y);
first = temp.*log(output);
second = (1-temp).*log(1-output);

answer = -(first+second);
tempTheta1 = Theta1(:,2:end);
tempTheta2 = Theta2(:,2:end);
extra = sum(sum(tempTheta1.^2,2),1)+ sum(sum(tempTheta2.^2,2),1);
extra = lambda*(extra/(2*m));


J  = (sum(sum(answer,2),1))/m + extra;
...
% gradient calculation.
	...
	delta_1 = (Theta1'*delta_2).*sigmoidGradient(a_1);
%display("op1 :");
